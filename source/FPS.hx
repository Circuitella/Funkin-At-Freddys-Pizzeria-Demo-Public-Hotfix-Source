...
class FPS extends TextField
{
	...
	**/
	...
	private #if !flash override #end function __enterFrame(deltaTime:Float):Void
	{
		...
		if (Type.getClassName(Type.getClass(FlxG.state)) == 'PlayState') { 
		...
		if (currentFPS > ClientPrefs.data.framerate / fpsMultiplier) currentFPS = ClientPrefs.data.framerate / fpsMultiplier;

			text = (ClientPrefs.data.showFPS ? "FPS: " + (ClientPrefs.data.ffmpegMode ? ClientPrefs.data.framerate : Math.round(currentFPS)) : "");
			
			if (ClientPrefs.data.showRamUsage) text += "\nMemory: " + CoolUtil.formatBytes(Memory.getCurrentUsage(), false, 2) + (ClientPrefs.data.showMaxRamUsage ? " / " + CoolUtil.formatBytes(Memory.getPeakUsage(), false, 2) : "");
